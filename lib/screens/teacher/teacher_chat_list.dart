// lib/screens/teacher/teacher_chat_list.dart
...
class _TeacherChatListPageState extends State<TeacherChatListPage> {
  ...
  Future<void> _startNewChat() async {
    ...
    try {
      final students = await _chatService.getUsersForChat(user.uid, 'teacher');

      if (!mounted) return;

      if (students.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No students available')),
        );
        return;
      }

      final selectedStudent = await showModalBottomSheet<Map<String, dynamic>>(
        context: context,
          ...
          await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) => ChatScreen(
                ...
                currentUserName: user.displayName ?? 'Teacher',
                userRole: 'teacher',
              ),
            ),
          );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<Map<String, dynamic>>>(
          stream: _chatService.getUserChats(user.uid, 'teacher'),
          initialData: const [],
